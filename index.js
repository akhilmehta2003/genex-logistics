document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ==========================================================================
     3. LOGISTICS HUB WIDGET - TAB SWITCHING
     ========================================================================== */
  ...
  /* ==========================================================================
     4. CONSIGNMENT TRACKING SIMULATOR
     ========================================================================== */
  ...
  // Custom mock tracking database
  const trackingData = {
    'GNX-10294': {
      origin: 'Mumbai (MH)',
      dest: 'Gurugram (HR)',
      status: 'In Transit',
      estArrival: 'June 18, 2026',
      progress: 66,
      steps: [
        { label: 'Booked', time: 'June 15 - 10:00 AM', completed: true },
        { label: 'Sorted', time: 'June 15 - 06:30 PM', completed: true },
        { label: 'In Transit', time: 'June 16 - 04:15 AM', active: true },
        { label: 'Delivered', time: 'Expected June 18', completed: false }
      ]
    },
    'GNX-88402': {
      origin: 'Chennai Port (TN)',
      dest: 'Bengaluru Warehouses (KA)',
      status: 'Delivered',
      estArrival: 'June 16, 2026',
      progress: 100,
      steps: [
        { label: 'Booked', time: 'June 14 - 08:00 AM', completed: true },
        { label: 'Sorted', time: 'June 14 - 03:00 PM', completed: true },
        { label: 'In Transit', time: 'June 15 - 02:00 AM', completed: true },
        { label: 'Delivered', time: 'June 16 - 11:30 AM', completed: true }
      ]
    },
    'GNX-DEFAULT': {
      origin: 'Delhi Hub (DL)',
      dest: 'Kolkata Cargo Hub (WB)',
      status: 'Out for Dispatch',
      estArrival: 'June 17, 2026',
      progress: 33,
      steps: [
        { label: 'Booked', time: 'June 16 - 09:00 AM', completed: true },
        { label: 'Sorted', time: 'June 16 - 05:00 PM', active: true },
        { label: 'In Transit', time: 'Expected June 17', completed: false },
        { label: 'Delivered', time: 'Expected June 18', completed: false }
      ]
    }
  };
  ...
  if (trackForm) {
    trackForm.addEventListener('submit', (e) => {
      e.preventDefault();
      const rawId = trackingInput.value.trim().toUpperCase();
      if (!rawId) return;

      // Select data (fallback to default if ID is not custom)
      const data = trackingData[rawId] || trackingData['GNX-DEFAULT'];
      
      // Update text fields
      document.getElementById('track-id-disp').innerText = rawId;
      document.getElementById('track-route-disp').innerText = `${data.origin} ➔ ${data.dest}`;
      document.getElementById('track-status-disp').innerText = data.status;
      document.getElementById('track-arrival-disp').innerText = data.estArrival;
      ...
      timelineSteps.forEach(step => {
        ...
        step.querySelector('.step-time').innerText = '';
      });

      // Animate progress timeline sequential load
      setTimeout(() => {
        progressBar.style.width = `${data.progress}%`;
        
        data.steps.forEach((stepInfo, idx) => {
          setTimeout(() => {
            const stepEl = timelineSteps[idx];
            if (stepInfo.completed) {
              stepEl.classList.add('completed');
            } else if (stepInfo.active) {
              stepEl.classList.add('active');
            }
            stepEl.querySelector('.step-time').innerText = stepInfo.time;
          }, idx * 250);
        });
      }, 150);
    });
